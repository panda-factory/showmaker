...
#include "third-party/skia/modules/skshaper/include/SkShaper.h"
#include "UTF8.h"
...
SkRect SelectionBox(const SkFontMetrics& metrics,
                     float advance,
                     SkPoint pos) {
...
void SetCharacterBounds(void* context,
                          const char* utf8Text,
                          size_t utf8TextBytes,
                          size_t glyphCount,
                          const SkGlyphID* glyphs,
                          ...
                          const SkFont& font)
{
    ...
    for (size_t glyphIndex = 0; glyphIndex < glyphCount; ++glyphIndex) {
        ...
        SkRect clusterBox = SelectionBox(metrics, clusterAdvances[0], clusterGlyphPositions[0]);
        for (unsigned i = 1; i < clusterGlyphCount; ++i) { // multiple glyphs
            clusterBox.join(SelectionBox(metrics, clusterAdvances[i], clusterGlyphPositions[i]));
        }
        ...
        int codePointCount = UTF::CountUTF8(utf8Text + textBegin);
        ...
        while (ptr < end) {  // for each codepoint in cluster
            ...
            nextPtr = UTF::NextUTF8(nextPtr, end);
                                     ...
                                     float width)
{
    ...
    if (textByteLen) {
        ...
        runHandler.setRunCallback(SetCharacterBounds, result.glyphBounds.data());
